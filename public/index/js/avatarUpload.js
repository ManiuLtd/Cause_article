var avatarUpload = function(options) {
	...
	function handleFiles(fn) {
		...
		//显示图片
		$("#bg").css("backgroundImage", "url(\"" + imgObjectURL + "\")");
		//储存原图片的尺寸
		var img = new Image();
	...
	function createImg(fn) {
		...
		img.onload = function () {
			...
			var imgs = new Image();
			imgs.src = URL;
			$file.siblings().find('.userimg').text('').append(imgs);
			$file.siblings().parent().parent().next().val(URL);
