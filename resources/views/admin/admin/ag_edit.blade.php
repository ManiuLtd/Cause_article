...
<div class="col-xs-12">
    ...
    <form class="form-horizontal" id="form" action="{{route('admin_group.update',['id'=>$group->id])}}">
        {{csrf_field()}}
        {{ method_field('PUT') }}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <input type="text" name="title" id="title" placeholder="用户组名" class="col-xs-10 col-sm-5" value="{{$group->title}}">
        ...
        <div class="form-group">
            ...
            <div class="control-label no-padding-left col-sm-1">
                <label>
                    <input name="state" id="status" @if($group->state == 1) checked="checked" @endif class="ace ace-switch ace-switch-2" type="checkbox" value="1" />
                    <span class="lbl"></span>
                </label>
            </div>
        </div>
                        ...
                        <div class="widget-box">
                            <div class="widget-header">
                                <h4 class="widget-title">
                                    <label>
                                        <input name="rule[]" class="ace ace-checkbox-2 father" @if(in_array($value['id'],$ruleid)) checked="checked" @endif type="checkbox" value="{{$value['id']}}"/>
                                        <span class="lbl" title="{{$value['title']}}"> {{subtext($value['title'],6)}}</span>
                                    </label>
                                </h4>
                            ...
                            <div class="widget-body">
                                <div class="widget-main row">
                                    ...
                                    <label class="col-xs-2" style="width:160px;">
                                        <input name="rule[]" class="ace ace-checkbox-2 children" @if(in_array($child['id'],$ruleid)) checked="checked" @endif type="checkbox" value="{{$child['id']}}"/>
                                        <span class="lbl" title="{{$child['title']}}"> {{subtext($child['title'],7)}}</span>
                                    </label>
                                    @if(!empty($child['children']))
                                        @foreach($child['children'] as $c)
                                        <label class="col-xs-2" style="width:160px;">
                                            <input name="rule[]" class="ace ace-checkbox-2 children" @if(in_array($c['id'],$ruleid)) checked="checked" @endif type="checkbox" value="{{$c['id']}}"/>
